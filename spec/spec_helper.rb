...
# this file to always be loaded, without a need to explicitly require it in any files.
#
...
RSpec.configure do |config|
  ...
  config.expect_with :rspec do |expectations|
    ...
    # and `failure_message` of custom matchers include text for helper methods defined using `chain`, e.g.:
  ...
  # rspec-mocks config goes here. You can use an alternate test double library (such as bogus or mocha) by changing the `mock_with` option here.
  ...
  # compatibility in RSpec 3). It causes shared context metadata to be inherited by the metadata hash of host groups and examples, rather than
