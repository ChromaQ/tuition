...
RSpec.describe User, type: :model do
  describe 'validations' do
    ...
    context 'has an employee_id' do
      it { is_expected.to validate_presence_of(:employee_id) }
    end

    context 'superuser' do
      it { is_expected.to allow_value(true, false).for(:superuser) }
    end

    context 'hr access role' do
      it { is_expected.to allow_value(true, false).for(:hr_access) }
    end
    context 'manager access role' do
      it { is_expected.to allow_value(true, false).for(:manager_access) }
    end
  end
