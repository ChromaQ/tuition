...
class CoursesController < ApplicationController
  ...
  def set_course
    @course = Course.includes(:approvals, :proofs, :reimbursement, goal: [:user, :school, :credential]).references(:goal, :school, :credential, :approvals, :user, :reimbursement, :proofs).find(params[:id])
  end

  # Only allow a trusted parameter "white list" through.
  def course_params
    params.require(:course).permit(:user_id, :employee_id, :goal_id, :start_date, :end_date, :course_title, :course_short, :credit_hours, :cost_estimate, :status)
  end
end
